...
export default new Vuex.Store({
    ...
    actions: {
        ...
        async loadPageAction({commit, state}) {
            const response = await messagesApi.page(++state.currentPage)
            const data = await response.json()
            commit('addMessagePageMutation', data.messages)
            commit('addMessagePageMutation', data.totalPages)
            commit('addMessagePageMutation', Math.min(data.currentPage, data.totalPages-1))
        }
    }
})